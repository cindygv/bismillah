...
const FormPengisian = () => {
  ...
  const handleSave = () => {
    // Validasi
    if (!namaLengkap || !jurusan || !fakultas || !seminar) {
      Alert.alert("Error", "Harap isi semua kolom formulir");
      return;
    }

    // Simpan data ke Firebase
    const databaseRef = firebase.database().ref("userseminar");
    const bookingData = {
      namaLengkap,
      jurusan,
      fakultas,
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.label}>Seminar yang diikuti:</Text>
      <TextInput
        style={styles.input}
        value={seminar}
        onChangeText={(text) => setSeminar(text)}
      />
